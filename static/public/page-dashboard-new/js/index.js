  ...
  const app = Vue.createApp({
    ...
    computed: {
      ...
      pages() {
        ...
        const startPage = Math.max(
          1,
          this.currentPages - Math.floor(maxPages / 2)
        );
      ...
      pagesreview() {
        ...
        const startPage = Math.max(
          1,
          this.currentPagereview - Math.floor(maxPages / 2)
        );
        const endPage = Math.min(
          this.totalPagesreview,
      ...
      pagesorder() {
        ...
        const startPage = Math.max(
          1,
          this.currentPagereorder - Math.floor(maxPages / 2)
        );
        const endPage = Math.min(
          this.totalPagesreorder,
          startPage + maxPages - 1
        );
      ...
      pagesreview2() {
        ...
        const startPage = Math.max(
          1,
          this.currentPagereview2 - Math.floor(maxPages / 2)
        );
        const endPage = Math.min(
          this.totalPagesreview2,
          startPage + maxPages - 1
        );
        for (let page = startPage; page <= endPage; page++) {
          pages.push(page);
    ...
    methods: {
      ...
      async init() {
        ...
        $("#page_size_select_review_script").on(
          "change.custom",
          async function () {
      ...
      async loadDataClip(page = 1, per_page = 10) {
        ...
        try {
          ...
          const currentDate = new Date();
          const formattedDate =
            currentDate.toISOString().slice(0, 10).replace("T", " ") +
            " 00:00:00";
      ...
      async loadDataProductChannel() {
        ...
        try {
          ...
          const responseGetOrderManual = await services.getOrderManual(
            data,
            this.token_header
          );
      ...
      async loadDataReview() {
        ...
        try {
          ...
          let responsegetClip = await services.getReview(
            data,
            self.token_header
          );
      ...
      async loadDataReviewTb2() {
        ...
        try {
          ...
          let responsegetTb2 = await services.getReviewTb2(
            data,
            self.token_header
          );
      ...
      async loadDailySum() {
        try {
          ...
          let formattedDatestart =
            this.startDate || new Date().toISOString().slice(0, 10); // Ensure you have a default value

          let formattedDateend =
            this.endDate || new Date().toISOString().slice(0, 10); // Ensure you have a default value
          let data = {
            start_at: formattedDatestart,
            end_at: formattedDateend,
          };
          const responseGetDailySum = await services.getDailySum(
            data,
            this.token_header
          );
          const response = responseGetDailySum?.data || {};
          this.dataDailySum = response.data || {};
          const matchedNames = this.dataNameChannel.map(
            (item) => item.channel_name
          );
          this.unmatchedItems = this.dataDailySum.filter(
            (item) => !matchedNames.includes(item.channel_name)
          );
          closeLoading();
        } catch (error) {
          console.warn("Error loading data:", error);
      ...
      async sortTableOrder(column) {
        if (this.column_order_order === column) {
          this.order_sort_order =
            this.order_sort_order === "asc" ? "desc" : "asc";
        } else {
      ...
      getSortIconorder(column) {
        ...
        return self.order_sort_order === "asc"
          ? "bi-chevron-up"
          : "bi-chevron-down";
      },
      ...
      changePagereview2(page) {
        if (
          page !== this.currentPagereview2 &&
          page > 0 &&
          page <= this.totalPagesreview2
        ) {
      ...
      changePagereview(page) {
        if (
          page !== this.currentPagereview &&
          page > 0 &&
          page <= this.totalPagesreview
        ) {
      ...
      filteredData(channelName) {
        ...
        return this.dataDailySum.filter(
          (list) => list.channel_name === channelName
        );
      },
      ...
      async filterModalVDO(value, tableSession) {
        ...
        console.log("🚀 ~ filterModalVDO ~ tableSession:", tableSession);
        const self = this;
        ...
        if (tableSession === 1) {
          if (value === "สินค้า") {
            ...
            try {
              ...
              self.data_channel = req.data.data.map((item) => {
                const existingProduct = self.filter_channel.find(
                  (prod) => prod.name === item
                );
          ...
          if (value === "ร้าน") {
            try {
              ...
              self.data_products_3 = req.data.data.map((item) => {
                const existingProduct = self.filter_products_3;
            ...
            try {
              const req = await services.ListInfractionReviewDaily(
                self.token_header
              );
              self.data_channel_3 = req.data.data.map((item) => {
                const existingProduct = self.filter_channel_3.find(
                  (prod) => prod === item
                );
          ...
          if (value === "ช่องทาง") {
            try {
              let data = {};
              const req = await services.ListChannelReviewNegative(
                data,
                self.token_header
              );
              self.data_channel_4 = req.data.data.map((item) => {
                const existingProduct = self.filter_channel_4.find(
                  (prod) => prod === item
                );
            ...
            try {
              const req = await services.ListProductReviewNegative(
                self.token_header
              );
              self.data_products_4 = req.data.data.map((item) => {
                const existingProduct = self.filter_products_4.find(
                  (prod) => prod === item
                );
            ...
            try {
              const req = await services.ListUserByReviewNegative(
                self.token_header
              );
              self.data_users_4 = req.data.data.map((item) => {
                const existingProduct = self.filter_users_4;
                return {
                  check_value: existingProduct
                    ? existingProduct.check_value
                    : false,
      ...
      async saveFilter() {
        ...
        if (self.modal_session === 1) {
          if (self.modal_titles === "สินค้า") {
            ...
            self.data_channel.forEach((item) =>
              addOrRemoveItem(self.filter_channel, item)
            );
          } else {
          ...
          if (self.modal_titles === "ร้าน") {
            self.data_products_3.forEach((item) =>
              addOrRemoveItem(self.filter_products_3, item)
            );
          } else if (self.modal_titles === "เรื่องการละเมิด") {
            self.data_channel_3.forEach((item) =>
              addOrRemoveItem(self.filter_channel_3, item)
            );
          } else {
            self.data_users_3.forEach((item) =>
              addOrRemoveItem(self.filter_users_3, item)
            );
          }
          ...
          if (self.modal_titles === "ช่องทาง") {
            self.data_channel_4.forEach((item) =>
              addOrRemoveItem(self.filter_channel_4, item)
            );
          } else if (self.modal_titles === "สินค้า") {
            self.data_products_4.forEach((item) =>
              addOrRemoveItem(self.filter_products_4, item)
            );
          } else if (self.modal_titles === "ผู้ดูแล") {
            self.data_users_4.forEach((item) =>
              addOrRemoveItem(self.filter_users_4, item)
            );
          } else {
            self.data_star_4.forEach((item) =>
              addOrRemoveItem(self.filter_star_4, item)
            );
          }
      ...
      async onSaveModal() {
        try {
          ...
          let response = await services.getUpdateActionReviewNegativeHandler(
            data,
            this.token_header
          );
